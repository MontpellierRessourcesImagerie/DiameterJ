...
// Creates iables with the values from the results table and saves the results table.							
...
// Conversion of iables to real units
	if(unit_conv == "Yes"){
		scale_pix = unit_pix;
		scale_unit = unit_real;
			};
...
//Analysis for Combining radius histograms
	if(batch_combo == "Yes") {
		...
		l1 = b.length;
	for(ij =0; ij<l1; ij++){ 
//Opens files of interest and parses them into new iable values					
...
//Opens Initial set of stored values and enters the data into iables				
		selectWindow("temp.csv");
			IJ.renameResults("Results");
				old_len = nResults;
